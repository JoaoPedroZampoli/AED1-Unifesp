...
PLista Muda(PLista L, int X1, int X2){
    PLista P, Q, PAnterior = NULL, QAnterior = NULL;
    for(P = L; P != NULL && P->Info != X1; PAnterior = P, P = P->Prox){
        for(Q = L; Q != NULL && Q->Info != X2; QAnterior = Q, Q = Q->Prox){
            if(P == NULL || Q == NULL){
                printf("\nElementos não Encontrados na Lista");
                return(L);
            }
        }
    }
